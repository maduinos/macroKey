...
**꽂고 앱을 켜면 끝입니다.** 이 보드는 첫 설치에도 손댈 것이 없습니다.

공장 상태로 스케치가 없으면 Caterina 부트로더가 그대로 떠 있어(`1b4f:9205` 등) 앱이 알아보고
바로 굽습니다. 스케치가 들어 있으면 그것이 여는 시리얼 포트에 앱이 **1200 bps 터치**를 보내
Caterina에 넣습니다. macroKey 펌웨어가 이미 돌고 있으면 `BOOT` 명령도 씁니다.
...
첫 설치에 필요한 동작이 아닙니다. 필요해지는 때는 **USB 자체가 죽었을 때**입니다 — 시리얼
포트를 열지 않는 펌웨어가 올라갔거나, 매크로가 폭주해 키를 무한히 뿜고 있을 때. 앱이 말을
걸 상대가 사라진 상황이라 1200 bps 터치가 통하지 않고, 이 더블탭이 유일한 길입니다.
그럴 때 앱은 `NeedsManualBootloader`와 함께 이 문장을 띄웁니다.
