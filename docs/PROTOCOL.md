...
- `t=record`는 **키 하나를 3초(`MK_RECORD_HOLD_MS`) 동안 단독으로 눌렀다**는 보고입니다.
  ...
  픽셀은 빨간데 아무것도 캡처되지 않는(혹은 그 반대의) 설명 불가능한 고장이 됩니다.
  ...
  아무것도 바인딩할 수 없습니다(§ 프로필 참조). 호스트는 이 이벤트를 시리얼 리더 스레드가
  아닌 별도 워커에서 처리해야 합니다. 처리 과정이 장치에 LED·프로필 쓰기를 요청하는데,
  그 응답을 배달할 수 있는 건 리더 스레드뿐이라 인라인 처리는 자기 자신을 기다리게 됩니다.
